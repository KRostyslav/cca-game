...
export default function StatsPage() {
  ...
  const { domains: allDomains, levels: allLevels } = trackContent(trackId);
  ...
  const download = () => {
    ...
    a.href = url;
    a.download = `${trackId === "architect" ? "cca" : "ccd"}-quest-save-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    <div className="mx-auto max-w-4xl px-5 py-12 sm:px-8">
      <header className="border-b border-hairline pb-8">
        ...
        <p className="mono mt-2 text-sm text-coral">
          {titleForLevel(progress.level, trackId)} · рівень {progress.level}
        </p>
      ...
      <section className="mt-14 border-t border-hairline pt-8">
        ...
        <p className="mt-2 max-w-2xl text-[0.9rem] text-parchment-dim">
          Прогрес тренажера {getTrack(trackId)?.label} зберігається окремо від іншого тренажера
          і живе лише в localStorage цього браузера. Очищення даних сайту зітре його — зробіть
          експорт, якщо шкода втратити.
        </p>
      ...
      <div className="mt-12">
        <ButtonLink href={href("/")}>← На карту</ButtonLink>
      </div>
    </div>
  );
}
