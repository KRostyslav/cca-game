...
export default function ExamRunPage() {
  ...
  const questions = useMemo(
    () => (session ? buildExam(trackId, session.seed) : []),
    [session, trackId],
  );
  ...
  const finish = useCallback(
    () => {
      ...
      const perDomain: Record<string, { correct: number; total: number }> = Object.fromEntries(
        trackContent(trackId).domains.map((d) => [d.id, { correct: 0, total: 0 }]),
      );
      const wrong: string[] = [];
      ...
      router.replace(href("/exam/result"));
    },
    [answers, questions, recordAnswer, recordExamRun, router, startedAt, trackId, href],
  );
    ...
    return (
      <div className="mx-auto max-w-2xl px-5 py-20 sm:px-8">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <ButtonLink href={href("/exam")} variant="ghost">
            Назад
          </ButtonLink>
        </div>
      </div>
