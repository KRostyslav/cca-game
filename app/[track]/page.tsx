...
export default function HomePage() {
  ...
  const allDomains = trackContent(trackId).domains;
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {nextId && (
                <ButtonLink href={href(`/play/${nextId}`)} variant="primary">
                  {store.stats.totalAnswers > 0 ? "Продовжити" : "Перший бій"} →
                </ButtonLink>
              )}
              <ButtonLink href={href("/train")}>Тренування</ButtonLink>
              {examUnlocked && <ButtonLink href={href("/exam")}>Симуляція екзамену</ButtonLink>}
            </div>
          </div>

          <dl className="panel grid grid-cols-2 gap-px self-start border-hairline bg-hairline">
            <Stat label="Рівень" value={String(progress.level)} note={titleForLevel(progress.level, trackId)} />
      ...
      <section className="mt-16">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            ...
            <div className="mt-6">
              {examUnlocked ? (
                <ButtonLink href={href("/exam")} variant="primary" className="w-full">
                  Скласти екзамен
                </ButtonLink>
              ) : (
