...
П'ять світів = п'ять доменів екзамену, 30 рівнів, 150 питань (кожне — англійською
...
3. Англійський дубль — у `content/domains/<домен>/questions.en.ts`: мапа `id питання → переклад`
   з полями `prompt`, `scenario?`, `explanation`, `choices` та `whyWrong`.
4. Прогнати `npm run validate:content` — він перевіряє схеми, зв'язність посилань,
   унікальність id, повноту англійського дубля (ключі варіантів мають збігатися 1:1)
   і відповідність розподілу питань вагам екзамену.
